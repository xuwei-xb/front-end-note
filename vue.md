...
> 参考答案：
>
> 放在 *methods* 里面。因为 *computed* 只有在它的相关依赖发生改变时才会重新求值。相比而言，方法只要发生重新渲染，*methods* 调用总会执行所有函数。 
...
> 1. compiler：编译器
> 2. core：和平台无关的通用运行时代码
> 3. platforms：平台专有代码
> 4. server：服务端渲染相关代码
> 5. sfc：单文件组件解析相关代码
> 6. shared：共享工具库代码
> **缺点**： 各个模块无法单独抽离出来下载安装，也无法针对单个模块进行发布

**vue3**
> Vue3 源码工程的搭建改为了 Monorepo 的形式，将模块拆分到了不同的包里面，
> 每个包有各自的 API、类型定义以及测试。这样一类粒度更细，责任划分更加明确
![alt text](image.png)
...
> RFC 是一种文档格式，它详细描述了某个特性或更改的提议，讨论其动机、设计选择、实现细节以及潜在的影响。在通过讨论和反馈达成共识后，
> RFC 会被采纳或拒绝。一份 RFC 主要的组成部分有
> 1. 标题：简短描述提案的目的
> 2. 摘要：简要说明提案的内容和动机
...
> 语法 API 优化
> 引入 RFC
> 源码优化体现在使用 typescript 重构整个 Vue 源码，对冷门的功能进行了删除，并且整个源码的结构变为了使用 Monorepo 进行管理，这样粒度更细，不同的包 可以独立测试发布。用户也可以单独引入某一个包使用，而不用必须引入 Vue.
> 
> 性能上的优化是整个 Vue3 最核心的变化，通过优化响应式、diff算法、模板编译，Vue3 的性能相比 Vue2 有质的飞跃，基本上将性能这一块儿做到了极致。所以Vue 的新项目建议都使用 Vue3 来搭建。
...
### 打包体积优化
1. Tree-shaking 支持：Vue 3 的模块化设计允许按需引入功能，未使用的 API 不会被打包，显著减少生产环境体积

### 新功能与语法调整
1. Fragment 支持：Vue 3 允许组件包含多个根节点，无需额外包裹元素
2. Teleport 与 Suspense：
    - Teleport 可将组件渲染到 DOM 的任意位置（如全局弹窗）。
    - Suspense 简化异步组件加载状态的处理
3. 生命周期钩子重命名：beforeDestroy 和 destroyed 改为 beforeUnmount 和 unmounted
4. 事件总线：
    
    | 特性                | Vue 2 事件总线（EventBus）       | mitt（Vue 3 推荐）               |
    |---------------------|----------------------------------|----------------------------------|
    | **API 设计**        | 依赖 Vue 实例的 `$emit`、`$on`   | 独立库（无框架耦合），提供 `emit`、`on`、`off` 方法 |
    | **作用域**          | 全局或局部 Vue 实例              | 独立实例，可创建多个事件中心（支持模块化隔离）        |
    | **TypeScript 支持** | 需手动扩展类型声明               | 原生支持泛型，提供完整类型定义（如 `Mitt<Events>`）  |
    | **内存管理**        | 需手动调用 `$off` 防止内存泄漏   | 支持批量移除（`off('*')`）或命名空间隔离             |
    | **体积**            | 内置于 Vue 运行时                | 仅 200B 轻量库（Tree-shaking 友好）                |
    | **跨框架兼容性**    | 仅限 Vue 生态                    | 支持任意框架（React、Vue 等）                       |
